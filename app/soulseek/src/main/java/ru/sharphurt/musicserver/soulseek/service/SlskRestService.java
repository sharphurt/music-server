...
public class SlskRestService {
    ...
    public SlskTransfersResponseDto postDownloadTask(String fileName, String userName, long size) {
        try {
            log.info("Creating download task. FileName: {}, UserName: {}, Size: {}", fileName, userName, size);
            ...
            if (response.getStatusCode().isError() || response.getBody() == null) {
                log.error("Не удалось создать задачу на загрузку трека. Soulseek вернул код {}. FileName: {}, UserName: {}",
            ...
            log.error("Не удалось создать задачу на загрузку трека. FileName: {}, UserName: {}", fileName, userName, e);
            return null;
        }
    }
    ...
    public Optional<SlskTransferDto> getDownloadByTransferId(String transferId) {
        try {
            SlskTransferDto slskDownloadsResultDto = restClient.get()
                .uri("/api/v0/transfers/downloads/transfer/" + transferId)
                .retrieve()
